...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def union(that: TweetSet): TweetSet =     left union (right union (that incl elem)) // 9ms O(n^0.9) b<<a
...
object GoogleVsApple {
  ...
  lazy val trending2: Tweet = (googleTweets union appleTweets).mostRetweeted
  }
...
//  println(GoogleVsApple.trending2)
}
